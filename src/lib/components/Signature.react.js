...
function SignatureCanvas({
   ...
   */
  const resizeCanvas = React.useCallback((newValue) => {
  ...
  React.useEffect(() => {
    ...
    signaturePadRef.current = new SignaturePad(signatureRef.current, { scale: -1 });
    ...
    return () => {
      ...
      signaturePadRef.current.removeEventListener('endStroke', strokeEndEvent)
    }
  }, [])
  React.useEffect(() => {
    if (disabled && signaturePadRef.current) {
  ...
  return (
    <div className='flex flex-col relative aspect-[5/2]'>
      ...
      <div className='absolute z-20 flex bottom-0 right-0 p-1 justify-end gap-2'>
        {disabled
          ...
          <button
            type='button'
            title='Edit'
            className={classNames('active:fill-blue-600 active:bg-grey-400/50 hover:bg-gray-300/50 p-1 rounded z-20 bg-white')}
          ...
          <>
            ...
            <button
              type='button'
              title='Redo'
              className={classNames('active:fill-blue-600 active:bg-grey-400/50 hover:bg-gray-300/50 p-1 rounded z-20 bg-white', redos.length === 0 && 'fill-gray-300  disabled:bg-transparent disabled:text-gray-300 active:fill-gray-300 active:bg-transparent active:text-gray-300')}
              disabled={redos.length === 0 || typeof value === 'string'}
              onClick={() => handleRedo()}
            >
            ...
            <button
              ...
              className={classNames('active:fill-blue-600 active:bg-grey-400/50 hover:bg-gray-300/50 p-1 rounded z-20 bg-white', signatureData === 0 && 'fill-gray-300  disabled:bg-transparent disabled:text-gray-300 active:fill-gray-300 active:bg-transparent active:text-gray-300')}
              onClick={() => {
                handleClear()
                handleDisable(false)
              }}
      ...
      <canvas className='aspect-[5/2] w-full border border-gray-500 min-h-40 relative z-10' ref={signatureRef} />
    </div >
  )
}
...
function Signature({ id, value = '', save = false, resize = false, setProps }) {
  ...
  const handleSave = React.useCallback(() => {
    if (signatureCanvasRef.current) {
      setProps({ value: signatureCanvasRef.current.toDataURL('image/svg+xml'), save: false })
    }
  }, [expanded]);
  ...
  const handleUndo = React.useCallback(() => {
    if (signatureCanvasRef.current) {
      ...
      setRedos((prev) => [...prev, { ...redo, height: oldHeight, width: oldWidth }])
  ...
  const handleRedo = React.useCallback(() => {
    if (signatureCanvasRef.current && redos?.length > 0) {
      ...
      const scaledData = scaleDataFrom(redoItem, signatureCanvasRef.current.signatureCanvas, redoItem.height, redoItem.width)
  ...
  const handleDisable = React.useCallback((state = false) => {
  ...
  React.useEffect(() => {
    signatureCanvasRef.current = expanded ? expandedSignatureCanvasRef.current : normalSignatureCanvasRef.current
